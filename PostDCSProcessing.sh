...
samtools mpileup -B -d 500000 -f $refGenome ${1/.aln.sort.bam/.filt.bam} | tee -a ${1/.aln.sort.bam/.filt.pileup} | python $progPath/CountMuts.py -d $3 -c $4 -C $5 > ${1%%.aln.sort.bam}.filt.pileup.d${3}-c${4}-${5}.countmuts
...
samtools mpileup -B -d 500000 -f $refGenome ${1/.aln.sort.bam/.filt.readgroups.clipped.bam} | tee -a ${1/.aln.sort.bam/.filt.readgroups.clipped.bam.pileup} | python $progPath/CountMuts.py -d $3 -c $4 -C $5 > ${1%%.aln.sort.bam}.filt.readgroups.clipped.bam.pileup.d${3}-c${4}-${5}.countmuts
